...
class FBAOutboundServiceMWS_Client implements FBAOutboundServiceMWS_Interface
{
	...
	public function createFulfillmentOrder($request)
	{
		...
		if(!empty($httpResponse['err_code']['status'])){
		return $httpResponse;
		}else{
	...
	private function _invoke(array $parameters)
	{
		try {
			...
			for (;;) {
				...
				if ($status != 200) {
					$err_code['err_code']['status'] = $status;
					var_dump($response['ResponseBody']);
					$err=(array) simplexml_load_string($response['ResponseBody']);
					$err_msg="";
					if($err['Error']){
						$err_msg=$err['Error']->Message;
					}
					$err_code['err_code']['message']=(string)$err_msg;
					return $err_code;
				}
